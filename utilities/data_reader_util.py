...
def read_excel_data(file_path: str, sheet_name: str = None):
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            testName, email, password, expected = row # comment two line 54 and 55
            data.append((str(testName),str(email or ""),str(password or ""),str(expected)))  # convert list to string
            #data.append(row)
    except Exception as e:
